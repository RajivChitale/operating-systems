...
std::atomic_flag lock = ATOMIC_FLAG_INIT; //the flag used for test and set
...
{
    ofstream fout;
    fout.open("output-tas.txt", ios_base::app); //open file in append mode
    fout << outstr; 
    fout.close();
}
...
int main()
{
    ...
    ofstream fout;
    fout.open("output-tas.txt"); //clears file
    fout << "TAS ME Output:" <<endl;
    fout.close();
